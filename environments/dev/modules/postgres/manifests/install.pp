class postgres::install {
  ...
  file { "$postgres_conf": ensure => present, mode => 0644, source => "$source/conf", require => File["/db"] }
  file { "$postgres_init": ensure => present, source => "$source/postgres_init", require => File["/db"] }
}